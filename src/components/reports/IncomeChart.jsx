...
const IncomeChart = ({ transactions }) => {
    ...
    const incomeData = Object.entries(groupedIncome)
        .map(([categoryId, data]) => ({
            category: getCategoryName(categoryId, categories, 'income'),
            amount: data.total,
            count: data.count,
    ...
    const maxAmount = Math.max(...incomeData.map(item => item.amount), 1);
        ...
        <div>
            <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                    <h4 className="text-lg font-medium text-gray-900">Income Breakdown</h4>
                ...
                {incomeData.map((item, index) => (
                    <div key={index} className="space-y-2">
                        ...
                        <div className="flex justify-between text-xs text-gray-500">
                            <span>{item.percentage.toFixed(1)}% of total income</span>
            ...
            {incomeData.length > 5 && (
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-600">
                        Showing top {Math.min(5, incomeData.length)} income categories.
                        {incomeData.length > 5 && ` ${incomeData.length - 5} more categories not shown.`}
                    </p>
                </div>
            )}
        </div>
